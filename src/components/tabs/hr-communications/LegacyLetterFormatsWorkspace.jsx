...
export default function LegacyLetterFormatsWorkspace({ employees, user, api, canManage }) {
  ...
  const saveDraft = async () => {
    ...
    setSaving(true)
    try {
      await api.createRecord(COMMUNICATION_KINDS.LETTER, { letterType: active.label, title: active.label, employeeId: selectedEmployee.id, employeeName: selectedEmployee.name, body: `${content.subject}\n\n${content.body}`, effectiveDate: variables.effectiveFrom || variables.documentDate, documentDate: variables.documentDate, promotedDesignation: activeFormat === 'promotion' ? variables.promotedDesignation.trim() : null, previousDesignation: activeFormat === 'promotion' ? (variables.currentDesignation.trim() || currentDesignation(selectedEmployee)) : null, version: 1, source: 'legacy_format_generator', formatId: active.id })
