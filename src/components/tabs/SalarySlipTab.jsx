...
export default function SalarySlipTab() {
  ...
  const columns = useMemo(() => [
    ...
    { id: 'holidays_group', header: 'HOLIDAYS', columns: [{ accessorKey: 'sunday', header: 'Sunday', size: 30 }, { accessorKey: 'holidays', header: 'Holiday', size: 30 }, { accessorKey: 'totalHolidays', header: 'Tot', size: 22 }] },
          ...
          <div className="max-w-full space-y-4 flex flex-col h-full overflow-hidden">
            <div className="flex justify-between items-center gap-4 bg-white p-2 rounded-lg border border-gray-200 shadow-sm shrink-0">
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-gray-100 rounded-md p-1"><button onClick={() => { const [y, m] = summaryMonth.split('-').map(Number); const d = new Date(y, m - 2, 1); setSummaryMonth(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`) }} className="p-1 hover:bg-white hover:shadow-sm rounded transition-all text-gray-600"><ChevronLeft size={12} /></button><div className="px-1.5 py-0.5 font-bold text-gray-900 text-[9px] min-w-[80px] text-center uppercase tracking-tighter">{new Date(summaryMonth + '-01').toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</div><button onClick={() => monthInputRef.current?.showPicker()} className="p-1 hover:bg-white hover:shadow-sm rounded transition-all text-indigo-600 relative"><CalendarIcon size={12} /><input ref={monthInputRef} type="month" value={summaryMonth} onChange={e => setSummaryMonth(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer pointer-events-none" /></button><button onClick={() => { const [y, m] = summaryMonth.split('-').map(Number); const d = new Date(y, m, 1); setSummaryMonth(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`) }} className="p-1 hover:bg-white hover:shadow-sm rounded transition-all text-gray-600"><ChevronRight size={12} /></button></div>
                <div className="h-4 w-px bg-gray-200 mx-0.5" /><button onClick={() => refetchSummary()} className="h-6 px-3 bg-gray-900 text-white font-bold rounded text-[7px] uppercase tracking-widest shadow hover:bg-black transition-all active:scale-95">Submit</button>
              </div>
                        ...
                        {table.getHeaderGroups().map((headerGroup, rowIdx) => (
                          <tr key={headerGroup.id}>
                            {headerGroup.headers.map(header => {
                              ...
                              const rowSpan = (header.column.columns.length === 0 && rowIdx === 0) ? 2 : 1;
                              return (
                                <th 
                                  key={header.id} 
