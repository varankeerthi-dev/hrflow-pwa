...
export default function HelpTab() {
  ...
  return <div className="mx-auto max-w-[1500px] animate-in fade-in slide-in-from-bottom-3 duration-300"><div className="mb-4 flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[0.14em] text-indigo-500">Help Center</p><h1 className="mt-1 text-2xl font-black tracking-tight text-gray-900">Help & module guide</h1><p className="mt-1 text-sm text-gray-500">Select a module on the left to view its guidance on the right.</p></div><div className="hidden items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-gray-500 md:flex"><Compass size={14} className="text-indigo-500" /> Two-pane reference</div></div><div className="grid min-h-[720px] grid-cols-1 gap-4 xl:grid-cols-[235px_minmax(0,1fr)]"><aside className="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm xl:sticky xl:top-4 xl:h-fit"><div className="mb-3 flex items-center gap-2 px-2 text-[11px] font-black uppercase tracking-[0.14em] text-gray-500"><Compass size={14} className="text-indigo-500" /> Modules</div><nav className="flex gap-1 overflow-x-auto xl:block xl:space-y-1">{HELP_MODULES.map(module => { const Icon = module.icon; const active = module.id === activeModuleId; return <button key={module.id} type="button" onClick={() => setActiveModuleId(module.id)} className={`group flex min-w-[180px] items-center gap-3 rounded-xl px-3 py-3 text-left transition-all xl:w-full ${active ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-200' : 'text-gray-600 hover:bg-indigo-50/80 hover:text-indigo-700'}`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${active ? 'bg-white/15 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-indigo-100 group-hover:text-indigo-600'}`}><Icon size={16} /></span><span className="min-w-0"><strong className="block truncate text-[12px] font-black">{module.label}</strong><small className={`mt-0.5 block truncate text-[10px] ${active ? 'text-white/70' : 'text-gray-400'}`}>{module.note}</small></span></button> })}</nav><div className="mt-4 hidden border-t border-gray-100 px-2 pt-4 text-[11px] leading-5 text-gray-400 xl:block">Attendance Policy opens the full Employee Field Guide. The other modules keep the existing quick-help content.</div></aside><main className="min-w-0">{activeModuleId === 'attendance-policy' ? <AttendancePolicyGuide /> : <FaqModuleView module={activeModule} />}</main></div></div>
}
